...
package body RL.Algorithms.DP is
   ...
   begin
      Put_Line("Policy: ");
      for S in State_Type loop
         Put_Line(S'Image & ": " & Policy(S)'Image);
      end loop;
   end Print_Policy;
   ...
   ) return Deterministic_Policy_Type is
   ...
   begin
      ...
      return Policy;
    end Policy_Iteration_With_Init;
   ...
   function Policy_Iteration(Model : DP_Model_Type; Discount_Factor : Float) return Deterministic_Policy_Type is
      ...
      Init_Value_Function : Value_Function_Type := (others => 0.0);
      Init_Policy : Deterministic_Policy_Type;

   begin
      Action_Random.Reset(Action_Gen);
      -- Initialize to random policy
      Init_Policy := (others => Action_Random.Random(Action_Gen));
      Print_Policy(Init_Policy);  -- TODO: Remove after debugging
      return Policy_Iteration_With_Init(Model, Discount_Factor, Init_Value_Function, Init_Policy);
